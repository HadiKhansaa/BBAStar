// a_star_gpu_grid_8dir_parallel_no_closed_list.cu
...
__global__ void aStarKernel(int *grid, int width, int height, int goalNodeId,
                            ...
                            int *nextOpenListBins, int *nextBinCounts,
                            unsigned int *nextBinBitMask,
                            bool *found, int *path, int *pathLength, int currentBin) {
        ...
        if (xNeighbor >= 0 && xNeighbor < width && yNeighbor >= 0 && yNeighbor < height) {
            ...
            if (grid[neighborId] == 0) {  // 0 indicates free cell
                ...
                if (tentativeG < oldG) {
                    ...
                    float minFValue = sqrt((float) (width*width + height*height));
                    ...
                    // debug
                    // printf("Added to bin: %d\n", binForNeighbor);

                    // Atomically add neighbor to the next bins
                    int position = atomicAdd(&nextBinCounts[binForNeighbor], 1);
                    if (position < MAX_BIN_SIZE) {
                        nextOpenListBins[binForNeighbor * MAX_BIN_SIZE + position] = neighborId;

                        // If position was 0, the bin was previously empty
                        if (position == 0) {
                            // Set the corresponding bit in nextBinBitMask
                            int maskIndex = binForNeighbor / 32;
                            unsigned int mask = 1U << (binForNeighbor % 32);
                            atomicOr(&nextBinBitMask[maskIndex], mask);
                        }
                    } else {
...
int main(int argc, char** argv) {
    ...
    // Initialize open list bins with double buffering
    int *d_currentOpenListBins, *d_nextOpenListBins;
    int *d_currentBinCounts, *d_nextBinCounts;

    CUDA_CHECK(cudaMalloc((void **)&d_currentOpenListBins, MAX_BINS * MAX_BIN_SIZE * sizeof(int)));
    CUDA_CHECK(cudaMalloc((void **)&d_nextOpenListBins, MAX_BINS * MAX_BIN_SIZE * sizeof(int)));
    CUDA_CHECK(cudaMalloc((void **)&d_currentBinCounts, MAX_BINS * sizeof(int)));
    CUDA_CHECK(cudaMalloc((void **)&d_nextBinCounts, MAX_BINS * sizeof(int)));

    CUDA_CHECK(cudaMemset(d_currentOpenListBins, -1, MAX_BINS * MAX_BIN_SIZE * sizeof(int)));
    CUDA_CHECK(cudaMemset(d_nextOpenListBins, -1, MAX_BINS * MAX_BIN_SIZE * sizeof(int)));
    CUDA_CHECK(cudaMemset(d_currentBinCounts, 0, MAX_BINS * sizeof(int)));
    CUDA_CHECK(cudaMemset(d_nextBinCounts, 0, MAX_BINS * sizeof(int)));

    // Initialize binBitMasks for double buffering
    unsigned int *d_currentBinBitMask, *d_nextBinBitMask;
    int binBitMaskSize = (MAX_BINS + 31) / 32;
    CUDA_CHECK(cudaMalloc((void **)&d_currentBinBitMask, binBitMaskSize * sizeof(unsigned int)));
    CUDA_CHECK(cudaMalloc((void **)&d_nextBinBitMask, binBitMaskSize * sizeof(unsigned int)));
    CUDA_CHECK(cudaMemset(d_currentBinBitMask, 0, binBitMaskSize * sizeof(unsigned int)));
    CUDA_CHECK(cudaMemset(d_nextBinBitMask, 0, binBitMaskSize * sizeof(unsigned int)));
    ...
    while (!h_found) {
        ...
        if (binSize > 0) {
            ...
            aStarKernel<<<blocksPerGrid, threadsPerBlock>>>(
                ...
                d_nextOpenListBins, d_nextBinCounts,
                d_nextBinBitMask,
                d_found,
                d_path, d_pathLength,
                currentBin);
        ...
        // Reset the current bin counts and bins
        h_binCounts[currentBin] = 0;
        CUDA_CHECK(cudaMemcpy(&d_currentBinCounts[currentBin], &h_binCounts[currentBin], sizeof(int), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemset(&d_currentOpenListBins[currentBin * MAX_BIN_SIZE], -1, MAX_BIN_SIZE * sizeof(int)));

        // Clear the corresponding bit in binBitMask
        int maskIndex = currentBin / 32;
        unsigned int mask = ~(1U << (currentBin % 32));
        h_binBitMask[maskIndex] &= mask;

        // Copy updated binBitMask to device
        CUDA_CHECK(cudaMemcpy(&d_currentBinBitMask[maskIndex], &h_binBitMask[maskIndex], sizeof(unsigned int), cudaMemcpyHostToDevice));

        // Swap current and next bins and binBitMasks
        std::swap(d_currentOpenListBins, d_nextOpenListBins);
        std::swap(d_currentBinCounts, d_nextBinCounts);
        std::swap(d_currentBinBitMask, d_nextBinBitMask);

        // Reset next bins and binBitMask for the next iteration
        CUDA_CHECK(cudaMemset(d_nextBinCounts, 0, MAX_BINS * sizeof(int)));
        CUDA_CHECK(cudaMemset(d_nextOpenListBins, -1, MAX_BINS * MAX_BIN_SIZE * sizeof(int)));
        CUDA_CHECK(cudaMemset(d_nextBinBitMask, 0, binBitMaskSize * sizeof(unsigned int)));
    }
    ...
    CUDA_CHECK(cudaFree(d_nextOpenListBins));
    CUDA_CHECK(cudaFree(d_currentBinCounts));
    CUDA_CHECK(cudaFree(d_nextBinCounts));
    CUDA_CHECK(cudaFree(d_currentBinBitMask));
    CUDA_CHECK(cudaFree(d_nextBinBitMask));
